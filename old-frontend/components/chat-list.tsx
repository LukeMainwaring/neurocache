...
import { IconFinancial, IconChevronDown } from './icons'
...
export function ChatList({ threadId, scrollRef }: ChatListProps) {
  const storeMessages = useMessagesStore(state => state.currentThread?.messages)
  ...
  const { data: currentCustomer } = useMyself()
  const [appSettings] = useAppSettings()
  const assistantId = appSettings.assistantId ?? AGILITY_DEFAULT_ASSISTANT_SLUG
  const history = useAssistantHistory(assistantId)

  // Inject a synthetic greeting message at the start of every conversation
  // This greeting is UI-only and not included in the actual message history
  // Show conversational survey greeting if plan not generated OR if this is the first thread
  const allMessages = useMemo(() => {
    // Check if this is the first thread in the history
    const isFirstThread =
      history && history.length > 0 && history[0]?.thread?.id === threadId

    const needsSurvey =
      isFirstThread || // Always show survey greeting for first thread
      !currentCustomer?.intake_survey ||
      !currentCustomer.intake_survey.plan_generated

    const greetingContent = needsSurvey
      ? "Hey Jamey — I'm [Uppy] and my goal is to partner with you on financial success. My job's simple: help you feel more in control of your money and less like it's running the show. Before we dive into numbers, can you tell me what feeling good about money would look like for you three months from now?"
      : 'Hi Jamey, welcome back!\n\nWhat would be most helpful to talk through right now?'

    const greetingMessage: StoreMessage = {
      localId: 'system-greeting',
      role: 'assistant',
    ...
    // Skip greeting if thread contains a proactive notification
    // Check both current messages store and persisted history store
    const hasProactiveNotificationInStore = storeMessages?.some(
      msg => msg.metadata?.isProactiveNotification === true
    )

    const currentThread = history?.find(
      h => h.thread?.id === threadId || h.localThreadId === threadId
    )
    const hasProactiveNotificationInHistory = currentThread?.messages?.some(
      msg => msg.metadata?.isProactiveNotification === true
    )

    if (hasProactiveNotificationInStore || hasProactiveNotificationInHistory) {
      return storeMessages || []
    }

    return !storeMessages || storeMessages.length === 0
      ? [greetingMessage]
      : [greetingMessage, ...storeMessages]
  }, [storeMessages, currentCustomer, history, threadId])
